...
const usernameValidations = (): ValidationChain => {
    return body('username')
        ...
        .isLength({max: 24})
        .withMessage('Username may not exceed 24 characters')
        .trim()
        .escape()
};
...
const passwordValidations = (): ValidationChain => {
    return body('password')
        ...
        .isLength({max: 32})
        .withMessage('Password may not exceed 32 characters')
        .matches(/\d/)
        ...
        .matches(/[a-z]/)
        .withMessage('Your password must contain a lower character.');
};
